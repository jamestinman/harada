-- Patch: task pin flag for Todo feed (client field `pinned` → column `tasks.pinned`)
-- Run against your Supabase/Postgres project after deploying app code that sends `pinned` in task rows.
-- Safe to run once; `IF NOT EXISTS` avoids errors if the column already exists.
